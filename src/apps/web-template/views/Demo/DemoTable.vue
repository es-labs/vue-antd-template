      ...
      <a-tabs v-model:activeKey="form.tabFormActiveKey">
        <a-tab-pane key="1" tab="Sort">
          <a-form layout="vertical">
            <a-form-item v-for="(col, index) of table2.columns" :key="index"> <a-switch v-model:checked="form.sorts[index]" /> {{ col.title }} </a-form-item>
          </a-form>
        </a-tab-pane>
        <a-tab-pane key="2" tab="Filter">
          <a-form layout="vertical">
            <a-form-item v-for="(col, index) of table2.columns" :key="index"> <a-switch v-model:checked="form.filters[index]" /> {{ col.title }} </a-form-item>
          </a-form>
        </a-tab-pane>
      </a-tabs>
    </a-drawer>
    <a-tabs v-model:activeKey="tabActiveKey">
      ...
      <a-tab-pane key="2" tab="Table 2">
        ...
        <a-table :scroll="table2.scroll" :columns="table2.columns" :data-source="table2.filteredData" @change="onChange2" rowKey="id">
          <!--
          <template #filterDropdown="{ setSelectedKeys, selectedKeys, confirm, clearFilters, column }"> </template>
          <template #filterIcon="filtered">
            <search-outlined />
          </template>
          -->
